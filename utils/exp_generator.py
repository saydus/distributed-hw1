...
class ExperimentGenerator ():
    ...
    def __init__(self, logger):
        ...
        # starting port num if multiple of the same service is deployed on the node
        self.disc_base_port = None
    ...
    def dump(self):
        ...
        self.logger.debug(
            "Base discovery port = {}".format(self.disc_base_port))
    ...
    def populate_dict(self, prefix, num_entities):
        ...
        if prefix == "disc":
            ...
            raise ValueError(
                "populate_dict::unknown prefix: {}".format(prefix))
        ...
        for i in range(num_entities):
            ...
            while (collision):
                ...
                if port:
                    # will be the case for disc and pubs
                    string = id + ":" + ip + ":" + str(port)
                else:
                ...
                if collision:
                    self.logger.debug(
                        "ExperimentGenerator::populate_dict -- collision occurred for string {}".format(str))

            # now that we know that the generated values do not cause collision
            # insert it into our dictionary
            target_dict[host].append(
                {"id": id, "hash": hash_val, "IP": ip, "port": port})
...
def parseCmdLineArgs():
    ...
    parser.add_argument("-D", "--num_disc_dht", type=int, default=20,
                        help="Number of Discovery DHT instances, default 20")

    parser.add_argument("-P", "--num_pub", type=int, default=5,
                        help="number of publishers, default 5")

    parser.add_argument("-S", "--num_sub", type=int, default=5,
                        help="number of subscribers, default 5")

    parser.add_argument("-d", "--disc_base_port", type=int,
                        default=5555, help="base port for discovery, default 5555")

    parser.add_argument("-p", "--pub_base_port", type=int,
                        default=7777, help="base port for publishers, default 7777")

    parser.add_argument("-t", "--mn_topo", default="single,20",
                        help="Mininet topology, default single,20 - other possibilities include linear,N or tree,fanout=N,depth=M")

    parser.add_argument("-f", "--script_file", default="mnexperiment.txt",
                        help="Experiment file to be sourced from Mininet prompt, default mnexperiment.txt")

    parser.add_argument("-j", "--json_file", default="dht.json",
                        help="JSON file with the database of all DHT nodes, default dht.json")
