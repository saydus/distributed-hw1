...
class SubscriberMW ():
    ...
    def handle_data(self):
        try:
            self.logger.info("SubscriberMW::handle_data")
    ...
    def event_loop(self, timeout=None):
        try:
            self.logger.info("SublisherMW::event_loop - run the event loop")
    ...
    def lookup(self):
        try:
            ...
            # what role we are playing, the list of topics we are listening to,
            ...
            self.logger.debug(
                "SubscriberMW::lookup - built the message")
            ...
            self.logger.info(
                "SubscriberMW::lookup - sent lookup message and now now wait for reply")
    ...
    def handle_reply(self):
        try:
            ...
            if (disc_resp.msg_type == discovery_pb2.TYPE_REGISTER):
                ...
                self.logger.info(
                    "SubscriberMW::handle_reply - lookup after topiclist subscription - publist = {}, type={}".format(
                        disc_resp.lookup_resp.publist, type(disc_resp.lookup_resp.publist))
                )
                for registrant_info in disc_resp.lookup_resp.publist:
